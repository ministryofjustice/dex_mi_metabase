...
function _build() {
  ...
  ecr_repo_name=dex-mi-metabase-ecr
  component=dex-mi-metabase-

  region='eu-west-2'
  context='live-1'
  aws_profile='ecr-live-1'

  git_remote_url="https://github.com/ministryofjustice/dex_mi_metabase.git";
  ...
  docker_build_tag=mi-${current_branch}-${short_version}
  ...
  p "------------------------------------------------------------------------"
  p "Building DEX MI image for deployment"
