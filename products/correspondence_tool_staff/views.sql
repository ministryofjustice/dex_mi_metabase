 ...
 SELECT stats_base_categories.sar_subject_type,
   ...
   FROM ( SELECT a.stats_month,
     ...
     LEFT JOIN ( SELECT warehouse_case_report_for_offender_sar_related.sar_subject_type,
          ...
          WHERE date_part('year'::text, warehouse_case_report_for_offender_sar_related.date_received) = date_part('year'::text, CURRENT_DATE) AND warehouse_case_report_for_offender_sar_related.case_type::text = 'Offender SAR'::text AND warehouse_case_report_for_offender_sar_related.case_originally_rejected::text != 'Yes'::text
     ...
     LEFT JOIN ( SELECT warehouse_case_report_for_offender_sar_related.sar_subject_type,
           ...
           FROM warehouse_case_report_for_offender_sar_related
          WHERE date_part('year'::text, warehouse_case_report_for_offender_sar_related.date_received) = (date_part('year'::text, CURRENT_DATE) - 1::double precision) AND warehouse_case_report_for_offender_sar_related.case_type::text = 'Offender SAR'::text AND warehouse_case_report_for_offender_sar_related.case_originally_rejected::text != 'Yes'::text
          GROUP BY warehouse_case_report_for_offender_sar_related.sar_subject_type, (date_part('year'::text, warehouse_case_report_for_offender_sar_related.date_received)), (date_part('month'::text, warehouse_case_report_for_offender_sar_related.date_received)), warehouse_case_report_for_offender_sar_related.requester_from
